...
! DATA STRUCTURE TO HOLD ALL NSST RECORDS.
 ...
 REAL, ALLOCATABLE, PUBLIC    :: TREF_GAUS(:,:)  !< GFS TREF on the gaussian grid.

 REAL, ALLOCATABLE, PUBLIC    :: TSFC_GAUS(:,:)  !< GFS TSFC on the gaussian grid.
 ...
 PUBLIC :: READ_SFCANL_DATA
 PUBLIC :: READ_SFCANL_TILE_DATA
 PUBLIC :: WRITE_DATA
...
!! @author George Gayno NCEP/EMC
 ...
 SUBROUTINE READ_SFCANL_DATA(SFCANL_FILE)
 ...
 ALLOCATE(DUMMY(IDIM_GAUS,JDIM_GAUS+2))
 ALLOCATE(TSFC_GAUS(IDIM_GAUS,JDIM_GAUS))
 ...
 DO J = 1, JDIM_GAUS
   TSFC_GAUS(:,J) = DUMMY(:,JDIM_GAUS+2-J)
 ENDDO
...
! REMOVE POLE POINTS. and flip latitude
 ...
 END SUBROUTINE READ_SFCANL_DATA
 ...
 !!
 !! @param[in] LENSFC Total number of points on a tile.
 !! @param[out] TMPT_TILE foundation temperature (TREF) or surface temperature (TSFC)
 !! @author Mingjing Tong NOAA/GFDL
 SUBROUTINE READ_SFCANL_TILE_DATA(LENSFC,TMPT_TILE,READ_TREF)
 ...
 LOGICAL, INTENT(IN)       :: READ_TREF
 REAL, INTENT(OUT)         :: TMPT_TILE(LENSFC)

 CHARACTER(LEN=50)         :: FNTILE
 CHARACTER(LEN=3)          :: RANKCH
 ...
 FNTILE = "./fntile." // RANKCH

 PRINT*
 PRINT*, "READ INPUT SFC DATA FROM: "//TRIM(FNTILE)

 ERROR=NF90_OPEN(TRIM(FNTILE),NF90_NOWRITE,NCID)
 CALL NETCDF_ERR(ERROR, 'OPENING FILE: '//TRIM(FNTILE) )
 ...
 IF (READ_TREF) THEN
   if (MYRANK == 0) PRINT*,'READ TREF'
   ERROR=NF90_INQ_VARID(NCID, "tref", ID_VAR)
   CALL NETCDF_ERR(ERROR, 'READING tref ID' )
   ERROR=NF90_GET_VAR(NCID, ID_VAR, DUMMY)
   CALL NETCDF_ERR(ERROR, 'READING tref' )
 ELSE
   if (MYRANK == 0) PRINT*,'READ TSFC'
   ERROR=NF90_INQ_VARID(NCID, "tsea", ID_VAR)
   CALL NETCDF_ERR(ERROR, 'READING tsea ID' )
   ERROR=NF90_GET_VAR(NCID, ID_VAR, DUMMY)
   CALL NETCDF_ERR(ERROR, 'READING tsea' )
 ...
 TMPT_TILE = RESHAPE(DUMMY, (/LENSFC/))

 DEALLOCATE(DUMMY)

 ERROR = NF90_CLOSE(NCID)

 END SUBROUTINE READ_SFCANL_TILE_DATA
