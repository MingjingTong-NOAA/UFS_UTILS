...
module use -a /ncrc/proj/epic/spack-stack/spack-stack-1.6.0/envs/unified-env/install/modulefiles/Core
...
if [ $EXTRACT_DATA == yes ]; then
  ...
  case $gfs_ver in
    v12 | v13)
      DATAH=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_${CDUMP} \
       -o log.data.${CDUMP} -e log.data.${CDUMP} ./get_pre-v14.data.sh ${CDUMP})
      DEPEND="-d afterok:$DATAH"
      if [ "$CDUMP" = "gdas" ] ; then
        DATA1=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_enkf \
    ...
    v15)
      ...
      if [ "$CDUMP" = "gfs" ] ; then
        ...
        DATA1=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp1 \
         -o log.data.grp1 -e log.data.grp1 ./get_v15.data.sh grp1)
        DATA2=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp2 \
         -o log.data.grp2 -e log.data.grp2 ./get_v15.data.sh grp2)
        DATA3=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp3 \
         -o log.data.grp3 -e log.data.grp3 ./get_v15.data.sh grp3)
        DATA4=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp4 \
         -o log.data.grp4 -e log.data.grp4 ./get_v15.data.sh grp4)
        DATA5=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp5 \
         -o log.data.grp5 -e log.data.grp5 ./get_v15.data.sh grp5)
        DATA6=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp6 \
         -o log.data.grp6 -e log.data.grp6 ./get_v15.data.sh grp6)
        DATA7=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp7 \
         -o log.data.grp7 -e log.data.grp7 ./get_v15.data.sh grp7)
        DATA8=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_grp8 \
      ...
      ;;
    v16retro)
      DATAH=$(sbatch --qos=normal --clusters=es --partition=rdtn_c5 --ntasks=1 -t $WALLT -A $PROJECT_CODE -J get_v16retro \
...
if [ $RUN_CHGRES == yes ]; then
  ...
  case $gfs_ver in
    v12 | v13)
      ...
      sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=6 --nodes=${NODES} --cpus-per-task=$OMP_NUM_THREADS \
        -t $WALLT -A $PROJECT_CODE -J chgres_${CDUMP} \
        -o log.${CDUMP} -e log.${CDUMP} ${DEPEND} run_pre-v14.chgres.sh ${CDUMP}
      ;;
    v14)
      sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=6 --nodes=${NODES} -t $WALLT -A $PROJECT_CODE -J chgres_${CDUMP} \
    ...
    v16retro)
      ...
      ;;
    v16)
      sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=24 --nodes=${NODES} -t $WALLT -A $PROJECT_CODE -J chgres_${CDUMP} \
      -o log.${CDUMP} -e log.${CDUMP} ${DEPEND} run_v16.chgres.sh ${CDUMP}
      ;;
  esac
  ...
  if [ "$CDUMP" = "gdas" ]; then
    ...
    if [ "$gfs_ver" = "v16retro" ]; then
      ...
      sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=24 --nodes=1 -t $WALLT -A $PROJECT_CODE -J chgres_enkf \
      ...
      while [ $MEMBER -le 80 ]; do
        ...
        case $gfs_ver in
          v12 | v13)
              ...
              export OMP_STACKSIZE=1024M
              sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=12 --nodes=1 --cpus-per-task=$OMP_NUM_THREADS \
               -t $WALLT -A $PROJECT_CODE -J chgres_${MEMBER_CH} \
               -o log.${MEMBER_CH} -e log.${MEMBER_CH} ${DEPEND} run_pre-v14.chgres.sh ${MEMBER_CH}
            ;;
          v14)
              sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=12 --nodes=1 -t $WALLT -A $PROJECT_CODE -J chgres_${MEMBER_CH} \
          ...
          v15)
            ...
            ;;
          v16)
              sbatch --qos=normal --clusters=c5 --partition=batch --ntasks-per-node=24 --nodes=1 -t $WALLT -A $PROJECT_CODE -J chgres_${MEMBER_CH} \
